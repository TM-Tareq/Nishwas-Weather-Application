...
import {
  Compass, Navigation2, MapPin, X, Loader2, Wind, Droplets,
  Thermometer, LocateFixed, ArrowUpDown, Clock, Ruler, AlertTriangle,
} from 'lucide-react';
  ...
  return (
    <div className="relative flex items-center gap-2" ref={ref}>
      <div className="relative flex-1">
        ...
        <input
          value={query} onChange={handleChange} onFocus={() => { if (suggestions.length) setOpen(true); }}
        ...
        {!loading && query && <button onClick={clear} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60"><X className="w-3.5 h-3.5" /></button>}
...
//  Map Explorer Tab
const MapExplorer = ({ geoLocation, mapRef }) => {
  ...
  const handleClick = async ({ lat, lng }) => {
    setClickedPos({ lat, lng }); setWeather(null); setAqi(null); setPanelLoad(true);
    try {
    ...
    <div className="flex flex-col h-full">
      ...
      {(clickedPos || panelLoad) && (
        <div className="glass-card p-4 mb-3 relative">
          <button onClick={() => { setClickedPos(null); setWeather(null); setAqi(null); }} className="absolute top-3 right-3 text-white/30 hover:text-white/60">
            <X className="w-4 h-4" />
          </button>
          {panelLoad ? (
            <div className="flex items-center gap-2 text-white/40 text-sm"><Loader2 className="w-4 h-4 animate-spin" /> {t('spatial.loading')}</div>
          ) : weather ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-emerald-400" />
                <p className="text-sm font-bold text-white">{weather.name}{weather.sys?.country ? `, ${weather.sys.country}` : ''}</p>
              </div>
              <div className="flex items-center gap-3">
                <img src={`https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`} alt="" className="w-12 h-12" />
                <div>
                  <p className="text-2xl font-extrabold text-white">{Math.round(weather.main.temp)}°C</p>
                  <p className="text-xs text-white/40 capitalize">{weather.weather[0].description}</p>
                </div>
                {aqiMeta && (
      ...
      {/* City AQI list */}
      <div className="mt-4 flex-1 overflow-y-auto space-y-1.5 scrollbar-hide">
        {cityData.map(city => {
...
//  Route Planner Tab 
const RoutePlanner = ({ geoLocation, currentWeather }) => {
  const [from, setFrom] = useState(null);
  const [to,   setTo]   = useState(null);
  const [routes, setRoutes]       = useState(null);
  const [routeLoading, setRL]     = useState(false);
  const [routeError,   setRE]     = useState('');
  ...
  const bestAirIdx = avgAqiPerRoute.reduce((bi, v, i) => {
    if (v == null) return bi;
    if (bi == null) return i;
    return v < avgAqiPerRoute[bi] ? i : bi;
  }, null);
  ...
  const findRoutes = async () => {
    ...
    try {
      const r = await fetchOsrmRoutes(from, to);
      setRoutes(r);
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      <LocInput icon={MapPin}    placeholder="From location…" value={from} onChange={setFrom} excludeLoc={to}   rightIcon={LocateFixed} onRightClick={handleLocate} rightLoading={gpsLoading} />
      <div className="flex items-center gap-2">
      ...
      >
        {routeLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Finding routes…</> : <><Wind className="w-4 h-4" /> Find Safest Route</>}
      </button>

      {routeError && <p className="text-red-400 text-sm glass-card px-3 py-2 rounded-xl">{routeError}</p>}

      {/* Route cards */}
      {routes && (
        <div className="flex-1 overflow-y-auto space-y-2.5 scrollbar-hide mt-1">
          {routes.map((route, i) => {
            ...
            return (
              <button
                key={i}
                onClick={() => setSelIdx(i)}
                className={`w-full text-left glass-card p-3.5 transition-all ${isSel ? 'border-emerald-500/40 neon-border' : 'hover:border-white/15'}`}
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex gap-1">
                    {i === bestAirIdx && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">Best Air</span>}
                    {i === fastestIdx && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">Fastest</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <div className="glass px-2 py-1.5 rounded-lg text-center">
                    <p className="text-[10px] text-white/30 uppercase">Time</p>
                    <p className="text-xs font-bold text-white">{fmtTime(route.duration)}</p>
                  </div>
                  <div className="glass px-2 py-1.5 rounded-lg text-center">
                    <p className="text-[10px] text-white/30 uppercase">Dist</p>
                    <p className="text-xs font-bold text-white">{fmtDist(route.distance)}</p>
                  </div>
                  <div className={`glass px-2 py-1.5 rounded-lg text-center ${meta?.bg ?? ''}`}>
                    <p className="text-[10px] text-white/30 uppercase">CAI</p>
...
const CityMarkers = ({ onAvgAqi }) => {
  ...
  useEffect(() => {
    Promise.allSettled(BD_CITIES.map(async c => {
      ...
      if (vals.length && onAvgAqi) onAvgAqi(vals.reduce((a, b) => a + b, 0) / vals.length);
    });
  }, []);
...
const SpatialHubPage = () => {
  ...
  const [mapWeather, setMapW]  = useState(null);
  const [mapAqi, setMapAqi]    = useState(null);
  const [mapPanelLoad, setMPL] = useState(false);
  ...
  const [routeData, setRouteData] = useState({ routes: null, from: null, to: null });

  const handleMapClick = async ({ lat, lng }) => {
    if (tab !== 'map') return;
    setMCPos({ lat, lng }); setMapW(null); setMapAqi(null); setMPL(true);
    try {
      const [w, a] = await Promise.allSettled([fetchCurrentWeather({ lat, lon: lng }), fetchAirQuality({ lat, lon: lng })]);
      setMapW(w.status === 'fulfilled' ? w.value : null);
      setMapAqi(a.status === 'fulfilled' ? a.value : null);
    } finally { setMPL(false); }
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 xl:w-96 flex-shrink-0 flex flex-col glass border-r border-white/8 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 scrollbar-hide">
            {tab === 'map' ? (
              <MapExplorer geoLocation={geoLocation} />
            ) : (
